package org.ranapat.resourceloader.libs
{
	import flash.display.LoaderInfo;
	import flash.utils.ByteArray;

	public function getDefinitionNames(data:Object, extended:Boolean = false, linkedOnly:Boolean = false):Vector.<String>
	{
		var bytes:ByteArray;
...
internal class Finder
{
	...
	public function getDefinitionNames(extended:Boolean, linkedOnly:Boolean):Vector.<String>
	{
		var definitions:Vector.<String> = new <String>[];
		var tag:uint;
		var id:uint;
		var length:uint;
...
internal class SWFByteArray extends ByteArray
{
	...
	public function readBits(length:uint, start:int = -1):Number
	{
		...
		var bit8:uint = this[super.position];
		var out:Number = 0;
		...
		if (bitsLeft > 0) {
			super.position++;
			out = this.readBits(bitsLeft, 0) | ((bit8 & ((1 << currentByteBitsLeft) - 1)) << (bitsLeft));
		} else {
			out = (bit8 >> (8 - length - start)) & ((1 << length) - 1);
	...
	{
		var result:uint = 0;
		var i:uint = 0, bit8:uint;
		do {
			bit8 = super.readUnsignedByte();
			result |= ( bit8 & 0x7F ) << ( i * 7 );
			i += 1;
		} while (bit8 & 1 << 7);
